...
public class FunctionDeclaration implements DeclarationInstruction {
    ...
    public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope) {
        if (_scope.accepts(this)) {
            ...
            SymbolTable _localScope = new SymbolTable(_scope);
            boolean isValid = true;
            for (ParameterDeclaration parameter : this.parameters) {
                if (_localScope.accepts(parameter)) {
                    _localScope.register(parameter);
                } else {
                    Logger.error("Function parameter error: '" + parameter.getName()
                            + "' is already defined in this function scope.");
                    isValid = false;
                }
            }
            isValid &= this.body.collectAndPartialResolve(_localScope, this);
            return isValid;
        } else {
            Logger.error("Function definition error: '" + this.name + "' is already defined.");
            return false;
        }
    }
	
	@Override
	public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope, FunctionDeclaration _container) { return this.collectAndPartialResolve(_scope); }
    ...
    public boolean completeResolve(HierarchicalScope<Declaration> _scope) {
        boolean isValid = true;
        for (ParameterDeclaration parameter : this.parameters) {
            isValid &= parameter.getType().completeResolve(_scope);
        }
        isValid &= this.type.completeResolve(_scope);
        isValid &= this.body.completeResolve(_scope);
        return isValid;
    }
	 ...
	 */
	@Override
	public boolean checkType() { return this.body.checkType(); }
    ...
    public int allocateMemory(Register _register, int _offset) {
        ...
        for (ParameterDeclaration parameter : this.parameters) {
            parameter.offset = paramOffset;
            parameter.register = Register.LB;
    ...
    public Fragment getCode(TAMFactory _factory) {
        this.label = "func_" + this.name + "_" + _factory.createLabelNumber();
        Fragment _result = _factory.createFragment();
        String _endLabel = "end_" + this.label;
        _result.add(_factory.createJump(_endLabel));
        Fragment _body = this.body.getCode(_factory);
        _body.addPrefix(this.label);
        _result.append(_body);
        int paramSize = 0;
        for (ParameterDeclaration parameter : this.parameters) {
            paramSize += parameter.getType().length();
        }
        _result.add(_factory.createReturn(this.type.length(), paramSize));
