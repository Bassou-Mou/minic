...
public class ConstantDeclaration implements DeclarationInstruction {
    ...
    public boolean collectAndPartialResolve(HierarchicalScope<Declaration> _scope) {
        if (_scope.accepts(this)) {
            ...
            Logger.error("Constant error: identifier '" + this.name + "' is already defined in this scope.");
            return false;
        }
    }
	 ...
	 */
	@Override
	public boolean completeResolve(HierarchicalScope<Declaration> _scope) { return this.value.completeResolve(_scope) && this.type.completeResolve(_scope); }
    ...
    public boolean checkType() {
        boolean isTypeValid = this.value.getType().compatibleWith(this.type);
        if (!isTypeValid) {
            Logger.error("Constant type error: '" + this.name
                    + "' expected type " + this.type
                    + " but got " + this.value.getType());
        }
        return isTypeValid;
    }
	/* (non-Javadoc)
	 ...
	 */
	@Override
	public Fragment getCode(TAMFactory _factory) { return this.value.getCode(_factory); }
