...
public class Return implements Instruction {
    ...
    public boolean checkType() {
        if (this.function == null) {
            Logger.error("Return error: instruction not associated with a function.");
            return false;
        }

        boolean isTypeValid = this.value.getType().compatibleWith(this.function.getType());

        if (!isTypeValid) {
            Logger.error("Return type mismatch: expected " + this.function.getType()
    ...
    public Fragment getCode(TAMFactory _factory) {
        ...
        _result.add(_factory.createReturn(this.function.getType().length(), paramSize));
        _result.addComment("RETURN instruction with value: " + this.value);
        return _result;
    }
