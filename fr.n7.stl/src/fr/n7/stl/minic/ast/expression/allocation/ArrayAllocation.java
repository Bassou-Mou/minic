...
public class ArrayAllocation implements AccessibleExpression, AssignableExpression {
    ...
    public boolean completeResolve(HierarchicalScope<Declaration> _scope) {
        boolean ok = this.element.completeResolve(_scope);
        ok &= this.size.completeResolve(_scope);
        // Vérifier que la taille est un entier
        if (!this.size.getType().compatibleWith(AtomicType.IntegerType)) {
            Logger.error("Array allocation: size must be an integer, got " + this.size.getType());
            ok = false;
        }
        return ok;
    }
    ...
    public Fragment getCode(TAMFactory _factory) {
        ...
        _fragment.append(this.size.getCode(_factory));
        _fragment.add(_factory.createLoadL(this.element.length()));
