...
public class ASTBuilder extends MiniCParserBaseListener {
    ...
    @Override
    public void exitInstructionFonctionLocale(InstructionFonctionLocaleContext ctx) {
        String name = ctx.Identificateur().getText();
        Type returnType = ctx.type().unType;
        List<ParameterDeclaration> parameters = ctx.parametres().lesParametres;
        Block body = ctx.bloc().unBloc;
        ctx.uneInstruction = new FunctionDeclaration(name, returnType, parameters, body);
    }
}
